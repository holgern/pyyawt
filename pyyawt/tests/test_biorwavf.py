...
class TestBiorwavf(unittest.TestCase):
    ...
    def test_sum(self):
        for N in np.arange(len(self.wnames)):
            ...
            np.testing.assert_almost_equal(np.sum(RF),np.sqrt(2), decimal=self.accuracy_decimal)
            np.testing.assert_almost_equal(np.sum(DF),np.sqrt(2), decimal=self.accuracy_decimal)

    def test_sumEven(self):
        for N in np.arange(len(self.wnames)):
            RF,DF = pyyawt.biorwavf(self.wnames[N])
            np.testing.assert_almost_equal(np.sum(RF[::2]), 1./np.sqrt(2), decimal=self.accuracy_decimal)
            np.testing.assert_almost_equal(np.sum(DF[::2]), 1./np.sqrt(2), decimal=self.accuracy_decimal)
