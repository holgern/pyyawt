...
def get_version_info():
    ...
    # write_version_py(), otherwise the import of pyyawt.version messes
    # up the build under Python 3.
    FULLVERSION = VERSION
    if os.path.exists('.git'):
        GIT_REVISION = git_version()
    elif os.path.exists('pyyawt/version.py'):
        # must be a source distribution, use existing version file
        # load it as a separate module to not load pywt/__init__.py
        import imp
        version = imp.load_source('pyyawt.version', 'pyyawt/version.py')
...
def write_version_py(filename='pyyawt/version.py'):
    cnt = """
# THIS FILE IS GENERATED FROM pyyawt SETUP.PY
...
def configuration(parent_package='',top_path=None):
    ...
    config.add_subpackage('pyyawt')

    config.get_version('pyyawt/version.py')
    return config
